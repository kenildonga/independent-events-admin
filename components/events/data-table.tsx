...
function DragHandle({ id, rowIndex }: { id: string; rowIndex: number }) {
...
export function DataTable({
    ...
    const filteredData = React.useMemo(() => {
        ...
        if (activeTab === "past-performance") return data.filter(event => event.isActive)
        if (activeTab === "key-personnel") return data.filter(event => !event.isActive)
        return data
    }, [data, activeTab])

    const updateStatus = (_id: string, isActive: boolean) => {
        setData(prev => prev.map(event => event._id === _id ? { ...event, isActive } : event))
    }
    ...
    const columns: ColumnDef<z.infer<typeof schema>>[] = [
        {
            id: "rowNumber",
            header: "#",
            cell: ({ row }) => <DragHandle id={row.original._id} rowIndex={row.index} />,
            enableSorting: false,
            enableHiding: false,
        },
        {
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="w-32">
                    {row.original.start_date ? new Date(row.original.start_date).toLocaleDateString() : 'Not specified'}
                </div>
            ),
        },
        {
            ...
            cell: ({ row }) => (
                <div className="w-32">
                    {row.original.end_date ? new Date(row.original.end_date).toLocaleDateString() : 'Not specified'}
                </div>
            ),
        },
        {
            ...
            cell: ({ row }) => (
                <div className="space-y-1">
                    <div>Boys: {row.original.required.boys}</div>
                    <div>Girls: {row.original.required.girls}</div>
                    <div>Any: {row.original.required.any}</div>
                </div>
            ),
        },
        {
            ...
            <div className="flex items-center justify-between px-4 lg:px-6">
                ...
                <Select value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <SelectContent>
                        ...
                        <SelectItem value="past-performance">Active Events</SelectItem>
                        <SelectItem value="key-personnel">Inactive Events</SelectItem>
                    </SelectContent>
                </Select>
